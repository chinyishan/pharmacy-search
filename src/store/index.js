...
export default createStore({
  ...
  getters: {
    ...
    // 區域選項，用 optional chaining 處理預設值 undefined
    districtList(state) {
      return (
        state.location.find((i) => i.name === state.currCity)?.districts || []
      );
    },
    //藥局縣市區篩選，比對是否有一樣的縣市區
    filteredStores(state) {
      const { stores } = state;
      const countyStore = stores.filter(
        (i) => i.county === state.currCity && i.town === state.currDistrict
      );
      return state.keywords
        ? countyStore.filter((i) => i.name.includes(state.keywords))
        : countyStore;
    },
    /** 目前地區資訊 */
    currDistrictInfo(state, getters) {
      return (
        getters.districtList.find((i) => i.name === state.currDistrict) || {}
      );
    },
  },
});
